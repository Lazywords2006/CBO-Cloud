...
def generate_chart_set_2():
    """生成第二组图表：任务规模 vs 成本"""
    try:
        print("🔄 开始生成图表集2 - 任务规模 vs 成本")
        print("=" * 60)
        ...
        # 检查图表集2的数据文件
        json_files = [f for f in os.listdir(raw_data_dir) if f.endswith('.json') and 'chart_set_2' in f]
        if len(json_files) < 4:  # 至少需要4个算法的数据
            print(f"❌ 图表集2数据不完整，只有 {len(json_files)} 个文件")
            return False
        ...
        # 生成图表
        print("\n📊 生成图表集2 - 任务规模 vs 成本...")
        chart_path = visualizer.create_figure10_task_scaling_analysis()
        
        if chart_path:
            print(f"✅ 图表集2生成成功: {chart_path}")
            
            # 生成完成报告（保存到tables文件夹）
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            tables_dir = os.path.join(results_dir, 'tables')
            os.makedirs(tables_dir, exist_ok=True)
            report_path = os.path.join(tables_dir, f"chart_set_2_report_{timestamp}.txt")
            with open(report_path, 'w', encoding='utf-8') as f:
                f.write("图表集2生成报告\n")
                f.write("=" * 30 + "\n\n")
                f.write(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"图表类型: 任务规模 vs 成本\n")
                f.write(f"数据文件数量: {len(json_files)}\n")
                f.write(f"生成文件: {chart_path}\n")
                f.write(f"使用的算法: BCBO, GA, PSO, ACO, FA, CS, GWO\n")
                f.write(f"任务规模范围: 100-1000个任务\n")
...
def main():
    """主函数"""
    print("🚀 图表集2生成工具")
    print("专门用于生成：任务规模 vs 成本图表")
    print("=" * 60)
    
    success = generate_chart_set_2()
