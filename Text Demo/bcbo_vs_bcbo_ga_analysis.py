...
from algorithm.BCBO_GA.bcbo_ga_cloud_scheduler import BCBO_GA_CloudScheduler


class BCBOvsBCBO_GA_Analyzer:
    """BCBO与BCBO-GA对比分析器"""

    def __init__(self):
        self.chart_data = {}
        self.bcbo_ga_data = None
        self.test_results = []
        self.load_all_data()
    ...
    def load_all_data(self):
        ...
        # 加载BCBO-GA测试数据
        try:
            bcbo_ga_file = os.path.join(current_dir, 'bcbo_ga_test_results.json')
            with open(bcbo_ga_file, 'r', encoding='utf-8') as f:
                self.bcbo_ga_data = json.load(f)
                print("[OK] 加载BCBO-GA测试数据")
        except Exception as e:
            print(f"[SKIP] BCBO-GA测试数据不存在: {e}")

    def run_comparison_tests(self, scenarios=None):
        """运行BCBO与BCBO-GA对比测试"""
        print("\n" + "="*70)
        print("执行BCBO vs BCBO-GA对比测试".center(70))
        print("="*70)
        ...
        for scenario in scenarios:
            ...
            bcbo_ga_start = time.time()
            bcbo_ga_result = bcbo_ga.optimize()
            bcbo_ga_time = time.time() - bcbo_ga_start
            bcbo_ga_fitness = bcbo_ga_result['best_fitness']

            # 计算改进率
            improvement = ((bcbo_ga_fitness - bcbo_fitness) / abs(bcbo_fitness)) * 100
            ...
            result = {
                ...
                'bcbo_ga_fitness': float(bcbo_ga_fitness),
                'improvement': float(improvement),
                'bcbo_time': float(bcbo_time),
                'bcbo_ga_time': float(bcbo_ga_time),
                'time_ratio': float(bcbo_ga_time / bcbo_time),
                'bcbo_ga_strategies': bcbo_ga_result.get('strategy_contributions', {})
            }
            self.test_results.append(result)

            # 显示结果
            print(f"  BCBO适应度:  {bcbo_fitness:.4f} (耗时: {bcbo_time:.2f}s)")
            print(f"  BCBO-GA适应度: {bcbo_ga_fitness:.4f} (耗时: {bcbo_ga_time:.2f}s)")
            print(f"  性能改进: {improvement:+.2f}%")
            print(f"  时间效率: {bcbo_ga_time/bcbo_time:.2f}x")

            if improvement > 0.1:
                print(f"  结果: BCBO-GA更优 [+]")
        ...
        # 保存测试结果
        output_file = os.path.join(current_dir, 'bcbo_vs_bcbo_ga_results.json')
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(self.test_results, f, indent=2, ensure_ascii=False)
            print(f"\n[OK] 测试结果已保存到 bcbo_vs_bcbo_ga_results.json")
    ...
    def analyze_performance_comparison(self):
        """分析性能对比"""
        print("\n" + "="*70)
        print("BCBO vs BCBO-GA 性能对比分析".center(70))
        print("="*70)
        ...
        print("\n性能对比汇总:")
        print("-"*70)
        print(f"{'场景':<15} {'BCBO':<15} {'BCBO-GA':<15} {'改进率':<12} {'结论':<10}")
        print("-"*70)

        wins = {'BCBO-GA': 0, 'BCBO': 0, 'Tie': 0}
        improvements = []

        for result in self.test_results:
            scenario = result['scenario']
            bcbo_fit = result['bcbo_fitness']
            bcbo_ga_fit = result['bcbo_ga_fitness']
            ...
            print(f"{scenario:<15} {bcbo_fit:<15.4f} {bcbo_ga_fit:<15.4f} {improvement:+.2f}% {conclusion:<10}")

        # 统计分析
        print("\n统计分析:")
        print("-"*50)
        print(f"  BCBO-GA获胜: {wins['BCBO-GA']} 次")
        ...
        if self.test_results[0].get('bcbo_time'):
            ...
            print("-"*50)
            for result in self.test_results:
                time_ratio = (result['bcbo_ga_time'] - result['bcbo_time']) / result['bcbo_time'] * 100
                print(f"  {result['scenario']}: BCBO-GA用时 {time_ratio:+.1f}% {'更多' if time_ratio > 0 else '更少'}")

    def analyze_bcbo_ga_strategies(self):
        """分析BCBO-GA策略贡献"""
        print("\n" + "="*70)
        print("BCBO-GA策略贡献度分析".center(70))
        print("="*70)

        if not self.test_results or not self.test_results[0].get('bcbo_ga_strategies'):
            print("[警告] 无策略数据")
            return
        ...
        for result in self.test_results:
            ...
            strategies = result.get('bcbo_ga_strategies', {})
            if strategies:
                # 排序策略
    ...
    def analyze_algorithm_characteristics(self):
        ...
        print("\n算法特性对比表:")
        print("-"*60)
        print(f"{'特性':<15} {'BCBO':<20} {'BCBO-GA':<20}")
        print("-"*60)

        for char in characteristics['BCBO'].keys():
            bcbo_val = characteristics['BCBO'][char]
            bcbo_ga_val = characteristics['BCBO-GA'][char]
            print(f"{char:<15} {bcbo_val:<20} {bcbo_ga_val:<20}")
    ...
    def generate_final_recommendation(self):
        ...
        if avg_improvement > 1.0:
            print(f"BCBO-GA平均改进率: {avg_improvement:+.2f}%")
            print("结论: BCBO-GA显著优于BCBO")
            winner = "BCBO-GA"
        elif avg_improvement > 0:
            print(f"BCBO-GA平均改进率: {avg_improvement:+.2f}%")
            print("结论: BCBO-GA略优于BCBO")
            winner = "BCBO-GA"
        ...
        if winner == "BCBO-GA":
            ...
            print("\n【工程实践应用】")
            print("  推荐算法: BCBO 或 BCBO-GA")
            print("  理由:")
            print("    - 小规模问题: BCBO (简单高效)")
            print("    - 大规模问题: BCBO-GA (性能更好)")
            print("    - 资源受限: BCBO (开销小)")
            print("    - 性能优先: BCBO-GA (效果好)")
    ...
    def plot_comparison_charts(self):
        ...
        bcbo_ga_fitness = [r['bcbo_ga_fitness'] for r in self.test_results]
        ...
        # 保存数据
        df.to_csv('bcbo_vs_bcbo_ga_comparison.csv', index=False, encoding='utf-8')
        print("\n[OK] 对比数据已保存到 bcbo_vs_bcbo_ga_comparison.csv")
    ...
    def run_full_analysis(self):
        """运行完整分析流程"""
        print("\n" + "="*80)
        print("BCBO vs BCBO-GA 完整对比分析".center(80))
        print("="*80)
        ...
        # 4. 分析BCBO-GA策略贡献
        self.analyze_bcbo_ga_strategies()
...
if __name__ == "__main__":
    analyzer = BCBOvsBCBO_GA_Analyzer()
